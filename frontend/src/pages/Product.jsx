...
const Product = () => {
  ...
  const fetchProductData = async () => {
    try {
      const response = await axios.get(backendUrl + `/api/v2/products/${productId}`, { withCredentials: true })
  ...
  if (!productData) return <div className='opacity-0 min-h-[60vh]'>Đang tải...</div>
    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      <div className='flex gap-12 flex-col sm:flex-row'>
        ...
        <div className='flex-1 flex flex-col-reverse gap-3 sm:flex-row'>
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {productData.images.map((item, index) => (
              <img onClick={() => setImage(item)} key={index} src={item} alt={productData.name}
                className={`w-[24%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer rounded transition-all ${image === item ? 'ring-2 ring-indigo-400' : 'hover:ring-2 hover:ring-gray-300'}`} />
            ))}
          </div>
        ...
        <div className='flex-1'>
          <div className='flex items-center gap-2 mb-3 flex-wrap'>
            <span className='bg-indigo-50 text-indigo-700 text-xs px-3 py-1 rounded-full font-medium'>{productData.brand}</span>
            <span className='bg-gray-100 text-gray-600 text-xs px-3 py-1 rounded-full'>{productData.category}</span>
          </div>

          <div className='flex items-start justify-between gap-4 mt-2'>
            <h1 className='font-bold text-2xl sm:text-3xl flex-1 text-gray-900 leading-tight'>{productData.name}</h1>
            <button type='button' onClick={() => toggleFavorite(productData._id)}
              ...
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                fill={isFavorite ? '#e11d48' : 'none'} stroke={isFavorite ? '#e11d48' : 'currentColor'} className='w-6 h-6'>
          ...
          <div className='flex items-center gap-1 mt-3'>
            {[1,2,3,4,5].map(star => (
              <img key={star} src={star <= averageRating ? assets.star_icon : assets.star_dull_icon} alt="star" className='w-4' />
            ))}
      ...
      <div className='mt-20'>
        <div className='flex border-b'>
          {[['description','Mô tả sản phẩm'], ['reviews',`Đánh giá (${reviews.length})`]].map(([key, label]) => (
            <button key={key} onClick={() => setActiveTab(key)}
              className={`px-6 py-3 text-sm font-medium transition-colors relative ${activeTab === key ? 'text-indigo-700 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
              {label}
            </button>
          ))}
        </div>
              ...
              <div className='mb-6'>
                ...
                <div className='bg-gray-50 rounded-xl overflow-hidden'>
                  {productData.specifications.map((spec, index) => (
                    <div key={index} className={`flex py-2.5 px-4 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
