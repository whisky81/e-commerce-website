...
const EMPTY_ADDR = { fullName: '', phone: '', street: '', ward: '', district: '', province: '', lat: null, lng: null, placeId: null }
...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      let response
      if (method === 'cod') {
        response = await axios.post(backendUrl + '/api/v2/orders/place', { addressId: selectedAddress._id }, { withCredentials: true })
        ...
        response = await axios.post(backendUrl + '/api/v2/orders/stripe', { addressId: selectedAddress._id }, { withCredentials: true })
        if (!response.data.success) {
          if (response.data.code === 'EMAIL_NOT_VERIFIED')
            ...
            <div className='rounded-xl p-4' style={{ background: '#EEF2FF', border: '1.5px solid #C7D2FE' }}>
              <div className='flex justify-between items-start'>
                <div>
                  ...
                  <p className='text-sm mt-1 text-gray-600'>{selectedAddress.street}, {selectedAddress.ward}, {selectedAddress.district}, {selectedAddress.province}</p>
        ...
        <div className='bg-white rounded-2xl border border-gray-200 p-6 shadow-sm sticky top-24 space-y-6'>
          ...
          <div>
            ...
            <div className='flex flex-col gap-3 mt-3'>
              {[
                { key: 'stripe', label: 'Thẻ quốc tế (Stripe)', icon: <img src={assets.stripe_logo} className='h-5' alt='stripe' /> },
                { key: 'cod',    label: 'Thanh toán khi nhận hàng', icon: <span className='text-lg'>💵</span> },
              ].map(({ key, label, icon }) => (
                <div key={key} onClick={() => setMethod(key)}
                  className={`flex items-center gap-4 border-2 rounded-xl p-4 cursor-pointer transition-all ${
                    method === key ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200 hover:bg-indigo-50/30'
                  }`}>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                    method === key ? 'border-indigo-500' : 'border-gray-300'
                  }`}>
                    {method === key && <div className='w-3 h-3 rounded-full' style={{ background: '#4F46E5' }} />}
                  </div>
                  {icon}
          ...
          <button type='submit' disabled={!selectedAddress || placing}
      ...
      {showAddressPopup && (
        <div className='fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm'>
          <div className='bg-white rounded-2xl max-w-2xl w-full max-h-[92vh] overflow-hidden shadow-2xl flex flex-col'>
            ...
            <div className='flex justify-between items-center p-5 border-b' style={{ background: 'linear-gradient(135deg,#F5F4FF,#EEF2FF)' }}>
              <h3 className='text-base font-bold' style={{ color: '#1E1B4B' }}>
                {showAddAddressForm ? '➕ Thêm địa chỉ mới' : '📍 Chọn địa chỉ giao hàng'}
              </h3>
              <button type='button' onClick={() => { setShowAddressPopup(false); setShowAddAddressForm(false); setShowMap(false) }}
                className='w-8 h-8 rounded-full hover:bg-indigo-100 transition-colors flex items-center justify-center text-gray-500 text-xl'>×</button>
            </div>
                  ...
                  {user?.addresses?.length === 0 ? (
                    ...
                    <div className='space-y-3'>
                      {user?.addresses?.map(address => (
                        <div key={address._id} onClick={() => { setSelectedAddress(address); setShowAddressPopup(false) }}
                          className={`border-2 rounded-xl p-4 cursor-pointer transition-all ${
                            selectedAddress?._id === address._id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-indigo-200'
                          }`}>
                          <div className='flex justify-between items-start'>
                            <div>
                              <div className='flex items-center gap-2 flex-wrap'>
                                <p className='font-semibold text-gray-900'>{address.fullName}</p>
                                {address.isDefault && <span className='text-xs px-2 py-0.5 rounded-full' style={{ background: '#EEF2FF', color: '#4F46E5' }}>Mặc định</span>}
                              </div>
                              <p className='text-sm text-gray-500 mt-0.5'>{address.phone}</p>
                              <p className='text-sm text-gray-500 mt-0.5'>{address.street}, {address.ward}, {address.district}, {address.province}</p>
                              {address.lat && <p className='text-xs mt-1' style={{ color: '#6366F1' }}>📍 Có tọa độ GPS</p>}
                            </div>
                            {selectedAddress?._id === address._id && (
                              <div className='w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0' style={{ background: '#4F46E5' }}>
                                <svg className='w-3 h-3 text-white' fill='currentColor' viewBox='0 0 20 20'><path fillRule='evenodd' d='M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z' /></svg>
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <div className='space-y-4'>
                  ...
                  {[
                    ...
                    <div key={name}>
                      ...
                      <input type={type} value={newAddress[name] || ''} placeholder={placeholder}
              ...
              {showAddAddressForm ? (
                <>
                  <button type='button' onClick={() => { setShowAddAddressForm(false); setShowMap(false) }}
                    className='px-4 py-2 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100'>Quay lại</button>
                  <button type='button' onClick={addAddr}
                    className='px-5 py-2 rounded-xl text-sm font-bold text-white hover:opacity-90'
                    style={{ background: 'linear-gradient(135deg,#4F46E5,#7C3AED)' }}>Lưu địa chỉ</button>
                </>
              ) : (
                <button type='button' onClick={() => setShowAddressPopup(false)}
                  className='px-4 py-2 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100'>Đóng</button>
              )}
            </div>
          </div>
        </div>
