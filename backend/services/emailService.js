...
export const sendOrderConfirmation = async ({
  to, name, orderId, items, totalPrice,
  welcomeDiscount = false, welcomeDiscountAmount = 0
}) => {
  ...
  const welcomeBanner = welcomeDiscount ? `
    <div style="background:#065F46;border-radius:10px;padding:12px 16px;margin:16px 0;text-align:center">
      <p style="color:#fff;margin:0;font-size:14px;font-weight:700">🎉 Ưu đãi chào mừng 20% đã được áp dụng!</p>
      <p style="color:#A7F3D0;margin:4px 0 0;font-size:12px">Giảm: ${fmt(welcomeDiscountAmount)}</p>
    </div>
  ` : "";
  ...
  await transporter.sendMail({
    ...
    to,
    subject: `✅ Xác nhận đơn hàng #${orderId} - ${SHOP_NAME}`,
    html: `
    <div style="background:#F8FAFC;padding:32px 16px;font-family:'Segoe UI',Arial,sans-serif">
      <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
        <div style="background:#000000;padding:32px 28px;text-align:center">
          <p style="color:#ffffff;margin:0 0 4px;font-size:20px;font-weight:800;letter-spacing:2px">🛒 ${SHOP_NAME}</p>
          <h1 style="color:#ffffff;margin:0;font-size:20px;font-weight:700">🎉 Đặt hàng thành công!</h1>
          <p style="color:#D1D5DB;margin:8px 0 0;font-size:13px">Cảm ơn bạn đã mua sắm tại ${SHOP_NAME}</p>
        </div>
        <div style="padding:28px">
          <p style="color:#374151;font-size:15px;margin:0 0 20px">Xin chào <strong>${name}</strong>,</p>
          <div style="background:#EFF6FF;border-left:4px solid #3B82F6;padding:12px 16px;border-radius:0 8px 8px 0;margin-bottom:16px">
...
export const sendVerificationEmail = async ({ to, name, token }) => {
  ...
  await transporter.sendMail({
    from: `"${SHOP_NAME}" <${process.env.SMTP_USER}>`,
    to,
    subject: `✉️ Xác nhận địa chỉ email - ${SHOP_NAME}`,
    html: `
    <div style="background:#F8FAFC;padding:32px 16px;font-family:'Segoe UI',Arial,sans-serif">
      <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
        <div style="background:#000000;padding:36px 28px;text-align:center">
          <p style="color:#ffffff;margin:0 0 8px;font-size:20px;font-weight:800;letter-spacing:2px">🛒 ${SHOP_NAME}</p>
          <h1 style="color:#ffffff;margin:0;font-size:22px;font-weight:700">Xác nhận Email</h1>
        </div>
        <div style="padding:32px 28px">
          <p style="color:#374151;font-size:15px;margin:0 0 8px">Xin chào <strong style="color:#1F2937">${name}</strong>!</p>
          <p style="color:#6B7280;font-size:14px;line-height:1.6;margin:0 0 24px">
            Cảm ơn bạn đã đăng ký tài khoản tại ${SHOP_NAME}. Vui lòng nhấn nút bên dưới để xác nhận địa chỉ email.
          </p>
          ...
          <p style="color:#9CA3AF;font-size:12px;margin:20px 0 0;line-height:1.5">
            ...
            <a href="${verifyUrl}" style="color:#4F46E5;word-break:break-all;font-size:11px">${verifyUrl}</a>
          </p>
        </div>
        <div style="background:#F8FAFC;padding:16px 28px;border-top:1px solid #E5E7EB;text-align:center">
...
export const sendPromotionEmail = async ({ to, name, subject, products, promoCode }) => {
  ...
  // ✅ Mỗi sản phẩm có thể click để đến trang chi tiết
  const productsHtml = products.slice(0, 6).map(p => {
    const productUrl = `${FRONTEND_URL}/product/${p._id}`;
    return `
    <a href="${productUrl}" style="display:inline-block;width:155px;margin:8px;vertical-align:top;
      ...
      <div style="padding:10px">
        ...
        <p style="font-size:14px;color:#111827;margin:0 0 6px;font-weight:700">${fmt(p.salePrice || p.price)}</p>
        ...
        <div style="margin-top:8px;background:#000000;color:#ffffff;text-align:center;
                    padding:6px;border-radius:6px;font-size:11px;font-weight:700">Xem ngay →</div>
      </div>
    </a>`;
  }).join("");
  ...
  await transporter.sendMail({
    from: `"${SHOP_NAME}" <${process.env.SMTP_USER}>`,
    to,
    subject: subject || `🔥 Ưu đãi đặc biệt từ ${SHOP_NAME}`,
    html: `
    <div style="background:#F8FAFC;padding:32px 16px;font-family:'Segoe UI',Arial,sans-serif">
      <div style="max-width:600px;margin:0 auto">
        <!-- Hero -->
        <div style="background:#000000;padding:40px 28px;border-radius:16px 16px 0 0;text-align:center">
          <p style="color:#ffffff;margin:0 0 8px;font-size:22px;font-weight:800;letter-spacing:2px">🛒 ${SHOP_NAME}</p>
          <h1 style="color:#ffffff;margin:0;font-size:28px;font-weight:800">🔥 Sale đặc biệt!</h1>
          <p style="color:#D1D5DB;margin:10px 0 0;font-size:15px">Xin chào <strong style="color:#fff">${name}</strong>, đừng bỏ lỡ ưu đãi này!</p>
        </div>

        <!-- Promo code -->
        ${promoCode ? `
        <div style="background:#fff;padding:20px 28px;border-left:4px solid #000;border-right:4px solid #000">
          ...
          <div style="background:#F5F3FF;border:2px dashed #000;border-radius:10px;padding:12px;text-align:center">
            <span style="font-size:26px;font-weight:800;color:#000;letter-spacing:4px;font-family:monospace">${promoCode}</span>
          </div>
        </div>` : ""}
        ...
        <div style="background:#fff;padding:24px 20px;${promoCode ? "" : "border-radius:0 0 16px 16px;"}">
          ...
          <div style="text-align:center;margin-top:24px">
            <a href="${FRONTEND_URL}/collection"
               style="display:inline-block;background:#000000;color:#ffffff;
                      text-decoration:none;padding:14px 36px;border-radius:10px;
        ...
        <div style="text-align:center;padding:20px;border-radius:0 0 16px 16px;background:#F1F5F9">
          <p style="color:#94A3B8;font-size:11px;margin:0">
            Bạn nhận được email này vì đã đăng ký nhận tin từ ${SHOP_NAME}.<br>
            <a href="${FRONTEND_URL}" style="color:#000">Truy cập cửa hàng</a>
          </p>
        </div>
      </div>
    </div>`,
