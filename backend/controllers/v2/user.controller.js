...
export const profile = async (req, res) => {
  try {
    const user = await User.findById(req.user._id)
      .select("name email addresses favorites isEmailVerified avatar");
    res.status(200).json({
      success: true,
      data: {
        id: user._id,
...
export const uploadAvatar = async (req, res) => {
  try {
    ...
    if (!file) return res.status(400).json({ success: false, message: "Không có file ảnh" });
...
export const listFavorites = async (req, res) => {
  try {
    const user = await User.findById(req.user._id)
      .populate({ path: "favorites", select: "_id name price images category brand stock soldCount discount salePrice" });
